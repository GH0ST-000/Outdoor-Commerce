...
import { addCalendarDays, monthRange, weekRange } from "@/features/map/lib/dates";
import { detailLevelForZoom, viewportExceedsDetail } from "@/features/map/lib/detail-level";
import { resolveMapConfig } from "@/features/map/lib/map-config";
import { createViewportSession, viewportRequestKey } from "@/features/map/lib/request-key";
import { buildShareUrl, shareUrlContainsPin } from "@/features/map/lib/share-link";
import {
...
describe("map url state", () => {
  it("ignores unsupported parameters and keeps a deterministic share link", () => {
    const parsed = parseMapSearchParams(
      new URLSearchParams("activity=flying&mode=date&date=2026-09-25&layers=nope,protected&z=99"),
      defaults,
    );
    expect(parsed.activity).toBe("hunting");
    expect(parsed.layers).toEqual(["protected"]);
    expect(parsed.z).toBe(6.5);
    const again = parseMapSearchParams(new URLSearchParams(serializeMapState(parsed)), defaults);
  ...
  it("rounds viewport coordinates and rejects an inverted box", () => {
    ...
    expect(normalizeBbox(43.1234, 41.1, 44.2, 42.2)).toEqual([43.123, 41.1, 44.2, 42.2]);
  });
  ...
  it("maps legal states and layer filters without color alone", () => {
    expect(paintForLegalState("unknown").mark).not.toBe(paintForLegalState("prohibited").mark);
    expect(paintForLegalState("conflict").dash).not.toEqual(paintForLegalState("prohibited").dash);
    expect(zoneTypesForLayers(["hunting", "protected"])).toContain("hunting_restricted_area");
    const fills = PROTECTED_CATEGORY_ORDER.map((zoneType) => protectedCategoryPaint[zoneType].fill);
    ...
    expect(displayLegalState("national_park", "hunting", "unknown")).toBe("prohibited");
    expect(displayLegalState("strict_nature_reserve", "hunting", "unknown")).toBe("prohibited");
    expect(displayLegalState("managed_reserve", "hunting", "unknown")).toBe("unknown");
    expect(displayLegalState("national_park", "fishing", "unknown")).toBe("unknown");
  });

  it("serializes periods and drops an incompatible species", () => {
    const state = { ...defaultMapState(defaults), mode: "range" as const, from: "2026-09-01", to: "2026-09-07" };
  ...
  it("formats attribution and strips coordinates from analytics", () => {
    ...
    expect(sanitizeMapEventPayload({ outcome: "unknown", lng: 44.1, pin: "1,2" })).toEqual({
      outcome: "unknown",
    });
  });
  ...
  it("builds calendar ranges in civil dates", () => {
    ...
    expect(weekRange("2026-09-25")).toEqual({ from: "2026-09-21", to: "2026-09-27" });
