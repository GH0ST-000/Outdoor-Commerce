...
/**
 * Restores a soft-deleted variant to draft. Never auto-activates, but it may
 * become the default when the product currently has none.
 */
final class RestoreProductVariantAction
{
    ...
    public function execute(
        ...
        $updated = DB::transaction(function () use ($variant, $actor, $requestId, $ipAddress, $userAgent): ProductVariant {
            ...
            $target = ProductVariant::withTrashed()->whereKey($variant->id)->lockForUpdate()->firstOrFail();
            $target->restore();
            $target->status = ProductVariantStatus::Draft;
            ...
            $this->recordAuditEvent->execute(new AuditEventData(
                event: AuditEvent::ProductVariantRestored,
                ...
                ipAddress: $ipAddress,
                userAgent: $userAgent,
                newValues: ['status' => ProductVariantStatus::Draft->value],
            ));
