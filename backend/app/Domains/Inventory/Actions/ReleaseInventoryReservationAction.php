...
final class ReleaseInventoryReservationAction
{
        ...
        return $this->deadlockRetry->run(function () use ($reservation, $idempotencyKey, $payloadHash, $releaseReason, $actorId, $requestId, $ipAddress, $userAgent): InventoryReservation {
            return DB::transaction(function () use ($reservation, $idempotencyKey, $payloadHash, $releaseReason, $actorId, $requestId, $ipAddress, $userAgent): InventoryReservation {
                ...
                $updated = $this->transitions->transition(
                    $reservation,
                    InventoryReservationTransition::Released,
                    $releaseReason,
                    null,
                    $afterCommit,
                );
                ...
                $this->recordAuditEvent->execute(new AuditEventData(
                    actorUserId: $actorId,
                    event: AuditEvent::InventoryReservationReleased,
                    ...
                    userAgent: $userAgent,
                    oldValues: null,
                    newValues: ['status' => InventoryReservationTransition::Released->value],
                    metadata: null,
                ));
